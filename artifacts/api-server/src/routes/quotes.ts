...
router.get("/", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const quotes = search
      ? await db.select().from(quotesTable).where(and(eq(quotesTable.bookId, bookId), ilike(quotesTable.text, `%${search}%`)))
...
router.post("/", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [quote] = await db.insert(quotesTable).values({
      ...
      text, page: page ?? null, chapter: chapter ?? null, color: color ?? "amber",
    }).returning();

    global.io?.to(`book-${bookId}`).emit("quote:created", {
      bookId, quoteId: quote.id, username: req.user!.username,
    });
...
router.patch("/:quoteId", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (isNaN(bookId) || isNaN(quoteId)) { res.status(400).json({ error: "Invalid ID" }); return; }

    const [existing] = await db.select().from(quotesTable).where(
      and(eq(quotesTable.id, quoteId), eq(quotesTable.bookId, bookId))
    ).limit(1);
...
router.delete("/:quoteId", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    global.io?.to(`book-${bookId}`).emit("quote:deleted", {
      bookId, quoteId, username: req.user!.username,
    });
